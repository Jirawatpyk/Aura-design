 ...
 * Usage: node scripts/fonts-check.mts   (CHROMIUM=/path/to/chrome to use a given browser) */
...
const TYPES: Record<string, string> = { '.css': 'text/css', '.woff2': 'font/woff2', '.html': 'text/html' };
const CSP = "default-src 'none'; style-src 'self'; font-src 'self'";
const page = (fonts: string) => `<!doctype html><html><head><meta charset="utf-8">
...
async function run(fonts: string) {
