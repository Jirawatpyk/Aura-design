...
import type { CalendarProps, DateDisplayOptions, DatePickerProps, DateRange, DateRangePickerProps, ISODate } from './types.js';
...
export type FormatDateOptions = DateDisplayOptions & { format?: 'short' | 'long' | 'numeric' | Intl.DateTimeFormatOptions };
...
export function toISO(d: Date | null | undefined): ISODate | null { return d ? d.getFullYear() + '-' + pad(d.getMonth() + 1) + '-' + pad(d.getDate()) : null; }
...
function same(a: Date | null | undefined, b: Date | null | undefined): boolean { return !!a && !!b && a.getFullYear() === b.getFullYear() && a.getMonth() === b.getMonth() && a.getDate() === b.getDate(); }
function localeTag(locale: string | null | undefined, calendar: string | null | undefined): string { return (locale === 'en' ? 'en-GB' : 'th-TH') + '-u-ca-' + (calendar === 'gregory' ? 'gregory' : 'buddhist'); }
var fmtCache: Record<string, Intl.DateTimeFormat> = {};
var PRESETS: Record<string, Intl.DateTimeFormatOptions> = { short: { day: 'numeric', month: 'short', year: 'numeric' }, long: { day: 'numeric', month: 'long', year: 'numeric' }, numeric: { day: '2-digit', month: '2-digit', year: 'numeric' } };
...
export function formatDate(iso: ISODate | null | undefined, opts?: FormatDateOptions): string {
  var o: FormatDateOptions = opts || {}, d = fromISO(iso);
  if (!d) return '';
  var f = typeof o.format === 'object' ? o.format : PRESETS[o.format || 'short'];
  return fmt(localeTag(o.locale, o.calendar), f, d)
    .replace(ERA, '');
}
var MONTHS: Record<string, number> | null = null;
function monthIndex(word: string): number {
  if (!MONTHS) {
    MONTHS = {};
    ['th-TH', 'en-GB'].forEach(function (tag: string) { (['short', 'long'] as const).forEach(function (w: 'short' | 'long') {
      for (var i = 0; i < 12; i++) {
...
export function parseDate(text: string | null | undefined): ISODate | null {
  ...
  var m = /^(\d{4})-(\d{1,2})-(\d{1,2})$/.exec(s), y: number, mo: number, d: number;
...
export const Calendar = React.forwardRef<HTMLDivElement, CalendarProps>(function Calendar(props, ref) {
  ...
  var viewState = React.useState<'days' | 'years'>('days'), view = viewState[0], setView = viewState[1];
  var hoverState = React.useState<Date | null>(null);
  var gridRef = React.useRef<HTMLDivElement | null>(null), gridMerged = useMergedRef(ref, gridRef), moved = React.useRef(false);
  var th = locale !== 'en';

  function disabled(d: Date): boolean {
    return !!((min && d < min) || (max && d > max) || (props.isDateDisabled && props.isDateDisabled(toISO(d) as ISODate)));
  }
  React.useEffect(function () {
    if (!moved.current) return;
    moved.current = false;
    var el = gridRef.current && gridRef.current.querySelector<HTMLElement>('[data-date="' + toISO(focusDate) + '"]');
    if (el) el.focus();
  });
  React.useEffect(function () {
    if (props.autoFocus === false) return;
    var el = gridRef.current && gridRef.current.querySelector<HTMLElement>('[tabindex="0"]');
    if (el) el.focus();
  }, []);
  function move(d: Date) { moved.current = true; setFocus(d); }
  ...
  var weekdayNames = weeks[0].map(function (d: Date) { return { short: fmt(tag, { weekday: th ? 'narrow' : 'short' }, d), long: fmt(tag, { weekday: 'long' }, d) }; });

  if (view === 'years') {
    var yr = focusDate.getFullYear(), base = yr - (yr % 12);
    var years: number[] = []; for (var y = base; y < base + 12; y++) years.push(y);
    return h('div', { className: 'aura-cal', ref: gridMerged },
      ...
      h('div', { className: 'aura-cal__years' }, years.map(function (y: number) {
  ...
  return h('div', { className: 'aura-cal', ref: gridMerged },
    ...
    h('table', { role: 'grid', className: 'aura-cal__grid', 'aria-label': monthTitle },
      h('thead', null, h('tr', null, weekdayNames.map(function (n: { short: string; long: string }, i: number) {
        return h('th', { key: i, scope: 'col', abbr: n.long }, h('span', { 'aria-hidden': true }, n.short));
      }))),
      h('tbody', { onMouseLeave: function () { hoverState[1](null); } }, weeks.map(function (wk: Date[], wi: number) {
        return h('tr', { key: wi }, wk.map(function (d: Date) {
          ...
          return h('td', { key: iso as string, role: 'gridcell', 'aria-selected': sel || undefined,
            ...
            h('button', {
              ...
              onKeyDown: function (e: React.KeyboardEvent) { onKey(e, d); },
...
function useCalendarPopover(boxRef: React.RefObject<HTMLDivElement | null>) {
  ...
  var pos = React.useState<PopoverPos | null>(null), popRef = React.useRef<HTMLDivElement | null>(null), mounted = useMounted();
  function place() {
...
interface DateFieldInternalProps {
  id: string; dialogId: string; label: string; hint?: React.ReactNode; error?: React.ReactNode; required?: boolean; optional?: boolean;
  disabled?: boolean; className?: string; name?: string; open: boolean; display: string; placeholder: string;
  boxRef: React.Ref<HTMLDivElement>; inputRef: React.Ref<HTMLInputElement>;
  hasValue: boolean; clearable?: boolean; onClear: () => void; clearLabel: string; toggleLabel: string;
  onCommit: (text: string) => void; onToggle: (open: boolean) => void;
}
  ...
  return h(Field, { id: props.id, label: props.label, hint: props.hint, error: props.error, required: props.required, optional: props.optional, disabled: props.disabled, className: props.className },
    h('div', { ref: props.boxRef, className: cx('aura-input aura-date', props.open && 'is-open') },
      h('input', {
        ...
        onChange: function (e: React.ChangeEvent<HTMLInputElement>) { setEditing(e.target.value); },
...
export const DatePicker = React.forwardRef<HTMLInputElement, DatePickerProps>(function DatePicker(props, ref) {
  ...
  var st = useMaybeControlled<ISODate | null>(props.value, props.defaultValue == null ? null : props.defaultValue, props.onChange);
  var boxRef = React.useRef<HTMLDivElement | null>(null), inputRef = React.useRef<HTMLInputElement | null>(null), inputMerged = useMergedRef(ref, inputRef);
  ...
  var cal = pop.open && pop.mounted && pop.pos ? createPortal(
    h('div', { ref: pop.popRef, id: dialogId, role: 'dialog', 'aria-modal': false, 'aria-label': props.label || (th ? 'เลือกวันที่' : 'Choose date'),
        className: 'aura-cal__popover', style: pop.pos, onKeyDown: function (e: React.KeyboardEvent) { if (e.key === 'Escape') { e.stopPropagation(); close(); } else trapTab(e, pop.popRef.current); } },
      h(Calendar, { locale: locale, calendar: calendar, weekStartsOn: props.weekStartsOn, min: props.min, max: props.max, isDateDisabled: props.isDateDisabled,
        start: st[0], focus: st[0], onSelect: function (iso: ISODate | null) { st[1](iso); close(); } })),
    document.body) : null;
  return h(React.Fragment, null,
    h(DateField, {
      ...
      onCommit: commit, onToggle: function (o: boolean) { pop.setOpen(o); if (!o && inputRef.current) inputRef.current.focus(); }
    }), cal);
});

/* DateRangePicker — a start and end date chosen on one calendar. value = { start, end } (ISO strings). */
export const DateRangePicker = React.forwardRef<HTMLInputElement, DateRangePickerProps>(function DateRangePicker(props, ref) {
  var auto = uid(), id = props.id || auto, dialogId = id + '-cal';
  var ctx = useAuraLocale(), locale = props.locale || ctx.locale || 'th', calendar = props.calendar || ctx.calendar || 'buddhist', th = locale !== 'en';
  var st = useMaybeControlled<DateRange>(props.value, props.defaultValue || { start: null, end: null }, props.onChange);
  var v = st[0] || { start: null, end: null };
  var draft = React.useState<ISODate | null>(null); /* start picked, waiting for end */
  var boxRef = React.useRef<HTMLDivElement | null>(null), inputRef = React.useRef<HTMLInputElement | null>(null), inputMerged = useMergedRef(ref, inputRef);
  var pop = useCalendarPopover(boxRef);
  ...
  function pick(iso: ISODate | null) {
    ...
    var a = draft[0], b = iso as ISODate;
  ...
  return h(React.Fragment, null,
    h(DateField, {
      ...
      onCommit: commit, onToggle: function (op: boolean) { pop.setOpen(op); draft[1](null); if (!op && inputRef.current) inputRef.current.focus(); }
    }), cal);
});
