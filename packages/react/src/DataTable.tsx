...
export const DataTable = React.forwardRef<HTMLDivElement, DataTableProps>(function DataTable(props, ref) {
  ...
  function onGridKey(e: React.KeyboardEvent) {
    ...
    if (r === 0 && col && e.altKey && (k === 'ArrowLeft' || k === 'ArrowRight') && canResize(col)) setW(col, widthOf(col)! + (k === 'ArrowLeft' ? -1 : 1) * (e.shiftKey ? 48 : 16), true);
      ...
      var inner = target.querySelector<HTMLElement>('button, a[href], input, select, textarea');
      if (inner) inner.focus(); else handled = false;
    }
    else if (k === ' ' || k === 'Enter') {
      ...
      else if (r > 0 && k === 'Enter' && target.querySelector('button, a[href], input, select, textarea')) target.querySelector<HTMLElement>('button, a[href], input, select, textarea')!.focus();
