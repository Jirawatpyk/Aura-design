...
function delin(c: number): number { return c <= 0.0031308 ? 12.92 * c : 1.055 * Math.pow(c, 1 / 2.4) - 0.055; }
...
function inGamut(rgb: number[]): boolean { return rgb.every(function (v) { return v >= -0.0005 && v <= 1.0005; }); }
...
function luminance(hex: string): number { var c = hexToRgb(hex).map(lin); return 0.2126 * c[0] + 0.7152 * c[1] + 0.0722 * c[2]; }
/** WCAG contrast ratio of two #hex colours. */
export function contrast(a: string, b: string): number { var x = luminance(a), y = luminance(b); return (Math.max(x, y) + 0.05) / (Math.min(x, y) + 0.05); }
function mix(a: string, b: string, t: number): string { var x = hexToRgb(a), y = hexToRgb(b); return rgbToHex(x.map(function (v, i) { return v + (y[i] - v) * t; })); }

/* AURA's lightness steps (from the violet and lime scales), and relative chroma. */
type ScaleStep = keyof Theme['brand'];
var STEPS: Record<ScaleStep, [number, number]> = { 50: [0.975, 0.18], 100: [0.945, 0.3], 200: [0.895, 0.5], 300: [0.81, 0.72], 400: [0.715, 0.9], 500: [0.635, 1], 600: [0.555, 1], 700: [0.49, 0.95], 800: [0.43, 0.85], 900: [0.38, 0.72] };
...
export function scale(hex: string): Theme['brand'] {
  var o = rgbToOklch(hexToRgb(hex)), C = Math.max(o[1], 0.02), out = {} as Theme['brand'];
  (Object.keys(STEPS) as unknown as ScaleStep[]).forEach(function (k) { out[k] = oklchToHex(STEPS[k][0], C * STEPS[k][1] / 0.95 * (o[1] < 0.03 ? 0.4 : 1), o[2]); });
  return out;
}
...
export function createTheme(opts: ThemeOptions): Theme {
  ...
  var L: Record<string, string> = {}, D: Record<string, string> = {};
  ...
  var checks: ThemeCheck[] = [];
  function check(theme: ThemeCheck['theme'], fg: string, bg: string, target: number, fgHex: string, bgHex: string): void { var r = contrast(fgHex, bgHex); checks.push({ theme: theme, pair: fg + ' on ' + bg, ratio: Math.round(r * 100) / 100, target: target, pass: r >= target - 0.005 }); }
  ...
  function css(selector?: string): string {
    ...
    function block(sel: string, m: Record<string, string>): string { return sel + ' {\n' + Object.keys(m).map(function (k) { return '  --aura-' + k + ': ' + m[k] + ';'; }).join('\n') + '\n}'; }
