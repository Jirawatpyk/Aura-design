...
export const FileUpload = React.forwardRef(function FileUpload(props, ref) {
  ...
  function add(list) {
    ...
    var room = maxFiles ? maxFiles - keep.filter(function (i) { return !i.error; }).length : Infinity;
  ...
  function remove(it) {
    ...
    st[1](items.filter(function (x) { return x.id !== it.id; }));
  ...
  return h(Field, { id: id, label: props.label, hint: props.hint, error: error, required: props.required, optional: props.optional, disabled: props.disabled, className: props.className },
    h('div', {
        ...
        onClick: function (e) { if (!props.disabled && e.target !== inputRef.current && inputRef.current) inputRef.current.click(); },
        onDragOver: function (e) { if (props.disabled) return; e.preventDefault(); if (!over) dragState[1](true); },
        onDragLeave: function (e) { if (!e.currentTarget.contains(e.relatedTarget)) dragState[1](false); },
        onDrop: function (e) { e.preventDefault(); dragState[1](false); add(e.dataTransfer && e.dataTransfer.files); } },
      ...
      h('input', { ref: inputMerged, id: id, type: 'file', className: 'aura-upload__input', accept: props.accept, multiple: !!props.multiple, disabled: props.disabled,
        name: props.name, required: props.required && !items.some(function (i) { return !i.error; }),
        ...
        onChange: function (e) { add(e.target.files); e.target.value = ''; } }),
