...
export var breakpoints: { sm: 640; md: 768; lg: 1024; xl: 1280 } = { sm: 640, md: 768, lg: 1024, xl: 1280 };
var ORDER: Breakpoint[] = ['base', 'sm', 'md', 'lg', 'xl'];

/** The widest breakpoint the window currently meets: 'base' | 'sm' | 'md' | 'lg' | 'xl'. 'lg' during server render. */
export function useBreakpoint(): Breakpoint {
  return React.useSyncExternalStore<Breakpoint>(subscribe, current, function () { return 'lg'; });
}
function current(): Breakpoint {
  var w = window.innerWidth, bp: Breakpoint = 'base';
  ORDER.slice(1).forEach(function (k) { if (w >= breakpoints[k as Exclude<Breakpoint, 'base'>]) bp = k; });
  return bp;
}
function subscribe(cb: () => void) { window.addEventListener('resize', cb); return function () { window.removeEventListener('resize', cb); }; }
...
export function useResponsive<T>(value: Responsive<T>): T | undefined {
  ...
  var map = value as Partial<Record<Breakpoint, T>>;
  for (var i = ORDER.indexOf(bp); i >= 0; i--) if (map[ORDER[i]] !== undefined) return map[ORDER[i]];
  return undefined;
}
function respVars<T>(prefix: string, value: Responsive<T> | null | undefined, map?: (v: T) => unknown): Record<string, unknown> {
  ...
  var byBp = value as Partial<Record<Breakpoint, T>>, cur: unknown;
  ORDER.forEach(function (k) { if (byBp[k] !== undefined) cur = map ? map(byBp[k] as T) : byBp[k]; if (cur !== undefined) style['--' + prefix + '-' + k] = cur; });
  return style;
}
var space = function (v: Space) { return typeof v === 'number' ? 'var(--aura-space-' + v + ')' : v; };
...
export const AppShell = React.forwardRef<HTMLDivElement, AppShellProps>(function AppShell(props, ref) {
  ...
  var navEl = props.nav as React.ReactElement<{ onChange?: (id: string) => void; className?: string }> | undefined;
  var nav = props.nav && React.isValidElement(props.nav) && compact
    ? React.cloneElement(navEl!, { onChange: function (id: string) { if (navEl!.props.onChange) navEl!.props.onChange(id); setOpen(false); }, className: cx(navEl!.props.className, 'is-in-drawer') })
    : props.nav;
