...
export const Menu = React.forwardRef<HTMLDivElement, MenuProps>(function Menu(props, ref) {
  var own = React.useRef<HTMLDivElement | null>(null), merged = useMergedRef(ref, own);
  var posState = React.useState<{ top: number; left: number } | null>(null);
  ...
  React.useEffect(function () {
    if (!mounted) return;
    var first = own.current && own.current.querySelector<HTMLElement>('[role^="menuitem"]:not([disabled])');
    if (first && props.autoFocus !== false) first.focus();
    function outside(e: Event) { if (own.current && !own.current.contains(e.target as Node) && !(props.anchor && props.anchor.contains(e.target as Node))) props.onClose(false); }
    function onScroll(e: Event) { if (own.current && !own.current.contains(e.target as Node)) props.onClose(false); }
  ...
  function onKeyDown(e: React.KeyboardEvent) {
    var list: HTMLElement[] = Array.prototype.slice.call(own.current!.querySelectorAll('[role^="menuitem"]:not([disabled])'));
    var i = list.indexOf(document.activeElement as HTMLElement);
