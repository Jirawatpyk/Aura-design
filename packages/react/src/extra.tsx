...
import type { AccordionItem, AccordionProps, BadgeProps, EmptyStateProps, PaginationProps, PopoverProps, ProgressProps, SkeletonProps, TagProps, Tone } from './types.js';
const h = React.createElement;
var TONES: string[] = ['neutral', 'accent', 'success', 'warning', 'danger'];
function tone(t: Tone | undefined): string { return TONES.indexOf(t as string) >= 0 ? t as string : 'neutral'; }
...
export const Tag = React.forwardRef<HTMLElement, TagProps>(function Tag(props, ref) {
  ...
  if (selectable) {
    return h('button', Object.assign({}, rest, { ref: ref as React.Ref<HTMLButtonElement>, type: 'button', 'aria-pressed': !!props.selected, disabled: props.disabled,
...
export const Skeleton = React.forwardRef<HTMLSpanElement, SkeletonProps>(function Skeleton(props, ref) {
  ...
  if (v === 'text' && (props.lines || 1) > 1) {
    var n = props.lines as number, rows: React.ReactElement[] = [];
  ...
  var style: { width?: number | string; height?: number | string } = { width: props.width, height: props.height };
...
function pageList(page: number, count: number, sib: number): Array<number | string> {
  var out: Array<number | string> = [], lo = Math.max(2, page - sib), hi = Math.min(count - 1, page + sib);
...
export const Pagination = React.forwardRef<HTMLElement, PaginationProps>(function Pagination(props, ref) {
  ...
  function go(p: number) { if (p >= 1 && p <= count && p !== page) st[1](p); }
  ...
  function item(p: number, label: React.ReactNode, extra?: Record<string, unknown>) {
    ...
    return link ? h('a', Object.assign({ href: link(p), onClick: function (e: React.MouseEvent) { if (props.onChange) { e.preventDefault(); go(p); } } }, common), label)
  ...
  return h('nav', { ref: ref, className: cx('aura-pagination', props.className), 'aria-label': props.label || t.pagination },
    ...
    h('ol', { className: 'aura-pagination__list' }, pageList(page, count, props.siblingCount == null ? 1 : props.siblingCount).map(function (p: number | string) {
...
export const Accordion = React.forwardRef<HTMLDivElement, AccordionProps>(function Accordion(props, ref) {
  ...
  var st = useMaybeControlled<string | null | string[]>(props.value, props.defaultValue != null ? props.defaultValue : multiple ? [] : null, props.onChange);
  var open: string[] = multiple ? ((st[0] as string[]) || []) : st[0] ? [st[0] as string] : [];
  var HT = 'h' + (props.headingLevel || 3);
  var items = props.items || [];
  function toggle(id: string) {
    var isOpen = open.indexOf(id) >= 0;
    if (multiple) st[1](isOpen ? open.filter(function (x: string) { return x !== id; }) : open.concat([id]));
    else st[1](isOpen ? (props.collapsible === false ? id : null) : id);
  }
  function onKey(e: React.KeyboardEvent<HTMLDivElement>) {
    var btns: HTMLElement[] = Array.prototype.slice.call(e.currentTarget.querySelectorAll(':scope > .aura-accordion__item > .aura-accordion__heading > button:not([disabled])'));
    var i = btns.indexOf(document.activeElement as HTMLElement), k = e.key, n: HTMLElement | null = null;
    if (i < 0) return;
...
function position(anchor: Element, pop: HTMLElement, placement: string | undefined): { top: number; left: number; side: string } {
...
export const Popover = React.forwardRef<HTMLDivElement, PopoverProps>(function Popover(props, ref) {
  ...
  var open = !!st[0];
  var wrap = React.useRef<HTMLSpanElement | null>(null), pop = React.useRef<HTMLDivElement | null>(null), popMerged = useMergedRef(ref, pop);
  var pos = React.useState<{ top: number; left: number; side: string } | null>(null), mounted = useMounted();
  function trigger(): HTMLElement | null { return wrap.current && (wrap.current.querySelector<HTMLElement>('button, [role="button"], a, input') || wrap.current.firstElementChild as HTMLElement | null); }
  function close(restore: boolean) { st[1](false); if (restore) { var tr = trigger(); if (tr && tr.focus) tr.focus(); } }
  ...
  React.useEffect(function () {
    ...
    if (props.autoFocus !== false && pop.current) { var f = pop.current.querySelector<HTMLElement>('[data-autofocus]') || pop.current.querySelector<HTMLElement>(FOCUSABLE); (f || pop.current).focus(); }
    function outside(e: Event) { if (pop.current && pop.current.contains(e.target as Node)) return; if (wrap.current && wrap.current.contains(e.target as Node)) return; close(false); }
  ...
  var panel = open && mounted ? createPortal(
    h('div', { ref: popMerged, id: id, role: 'dialog', 'aria-modal': false, 'aria-label': props.title ? undefined : props.label, 'aria-labelledby': props.title ? id + '-title' : undefined,
      ...
      onKeyDown: function (e: React.KeyboardEvent) { if (e.key === 'Escape') { e.stopPropagation(); close(true); } else trapTab(e, pop.current); } },
  ...
  return h('span', { ref: wrap, className: 'aura-popover-anchor' },
    React.cloneElement(child, { onClick: function (e: React.MouseEvent) { if (child.props.onClick) child.props.onClick(e); st[1](!open); }, 'aria-haspopup': 'dialog', 'aria-expanded': open, 'aria-controls': open ? id : undefined }),
    panel);
});
