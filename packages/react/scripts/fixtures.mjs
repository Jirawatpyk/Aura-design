...
export function fixtures(A, React) {
  ...
  const rows = [{ id: 'BK-1042', name: 'คุณสมชาย ใจดี', status: 'Ready', owner: 'Tao' }, { id: 'BK-1043', name: 'Anna Lee', status: 'Blocked', owner: 'Mai' }];
  const noop = () => {};
  return {
    Icon: h(A.Icon, { name: 'calendar' }),
    Button: h(A.Button, { icon: 'plus' }, 'New booking'),
    ...
    Select: h(A.Select, { label: 'Service', options: ['Deep clean', 'Regular'] }),
    RadioGroup: h(A.RadioGroup, { label: 'Frequency', options: ['Once', 'Weekly'], defaultValue: 'Once' }),
    Switch: h(A.Switch, { label: 'Notify' }),
    Combobox: h(A.Combobox, { label: 'แม่บ้าน', options: [{ value: 'a', label: 'สมศรี', keywords: ['somsri'] }, 'Mai'], defaultValue: 'a', clearable: true }),
    ...
    Alert: h(A.Alert, { tone: 'warning', title: 'Heads up' }, 'Two bookings overlap.'),
    Toaster: h(A.Toaster, null),
    Tooltip: h(A.Tooltip, { content: 'Copy' }, h(A.IconButton, { icon: 'copy', label: 'Copy' })),
    Dialog: h(A.Dialog, { open: true, onClose: noop, title: 'Cancel booking?' }, 'Body'),
    Drawer: h(A.Drawer, { open: true, onClose: noop, title: 'BK-1042' }, 'Detail'),
    ...
    Breadcrumb: h(A.Breadcrumb, { items: [{ label: 'Bookings', href: '#' }, { label: 'BK-1042' }] }),
    ...
    AppShell: h(A.AppShell, { nav: h(A.SideNav, { items: [{ id: 'd', label: 'Dashboard' }], value: 'd' }), header: 'Bookings' }, 'content'),
    ...
    Stat: h(A.Stat, { label: 'Bookings', value: '42', unit: 'งาน', change: { value: '+12%', direction: 'up', label: 'vs last month' }, icon: 'calendar' }),
    ...
    Tag: h(A.Tag, { onRemove: noop }, 'สุขุมวิท'),
